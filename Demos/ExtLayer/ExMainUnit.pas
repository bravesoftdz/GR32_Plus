...
type
  ...
  protected
    RBLayer: TExtRubberBandLayer;
    function CreatePositionedLayer: TTransformationLayer;
...
begin
  ...
  CreateNewImage(400,300, clWhite32);
...
function TMainForm.CreatePositionedLayer: TTransformationLayer;
var
  P: TPoint;
begin
  ...
  Result := TTransformationLayer.Create(ImgView.Layers);
  Result.Position := FloatPoint(P);// FloatRect(P.X - 32, P.Y - 32, P.X + 32, P.Y + 32);
...
begin
  if Selection is TExtBitmapLayer then
    TExtBitmapLayer(Selection).Cropped := CbxCropped.Checked;
end;
...
begin
  if Selection is TExtBitmapLayer then
  begin
    RESAMPLER[CbxLayerInterpolate.Checked].Create(TExtBitmapLayer(Selection).Bitmap);
  end;
end;

procedure TMainForm.LayerDblClick(Sender: TObject);
begin
  if Sender is TExtRubberBandLayer then
...
begin
  if Selection is TExtBitmapLayer then
    TExtBitmapLayer(Selection).Bitmap.MasterAlpha := GbrLayerOpacity.Position;
end;
...
begin
  ...
  MnuScaled.Enabled := B;
  MnuScaled.Checked := B and Selection.ScaledViewport;
  MnuDelete.Enabled := B;
  B := B and (Selection is TExtBitmapLayer);
...
const
  CScale = 1 / 200;
begin
  if Sender is TTransformationLayer then
    with TTransformationLayer(Sender) do
    begin
...
begin
{  if Sender is TTransformationLayer then
    with TTransformationLayer(Sender).GetAdjustedLocation do
    begin
      W2 := (Right - Left) * 0.5;
      H2 := (Bottom - Top) * 0.5;
      Cx := Left + W2;
...
procedure TMainForm.SetSelection(Value: TTransformationLayer);
begin
...
begin
  if Selection is TExtBitmapLayer then
    TExtBitmapLayer(Selection).Bitmap.FlipHorz;
end;

procedure TMainForm.MnuFlipVertClick(Sender: TObject);
begin
  if Selection is TExtBitmapLayer then
    TExtBitmapLayer(Selection).Bitmap.FlipVert;
end;
...
{  if Selection is TExtBitmapLayer then
  begin
    ...
    TExtBitmapLayer(Selection).Bitmap.Rotate90;
...
begin
  if Selection is TExtBitmapLayer then
    TExtBitmapLayer(Selection).Bitmap.Rotate180;
end;
...
{  if Selection is TExtBitmapLayer then
  begin
    R := Selection.Location;
    TExtBitmapLayer(Selection).Bitmap.Rotate270;
    Cx := (R.Left + R.Right) * 0.5;
    Cy := (R.Top + R.Bottom) * 0.5;
    W2 := (R.Right - R.Left) * 0.5;
    H2 := (R.Bottom - R.Top) * 0.5;
